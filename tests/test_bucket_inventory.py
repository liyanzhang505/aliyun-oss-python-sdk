...
class TestBucket(OssTestCase):
    def test_bucket_inventory(self):
        ...
        result = self.bucket.get_bucket_inventory_configuration(configuration_id = configuration_id);
        self.assertEquals(configuration_id, result.id)
        self.assertEquals(True, result.is_enabled)
        self.assertEquals(INVENTORY_FREQUENCY_DAILY, result.inventory_schedule.frequency)
        self.assertEquals(INVENTORY_INCLUDED_OBJECT_VERSIONS_CURRENT, result.included_object_versions)
        ...
        self.bucket.delete_bucket_inventory_configuration(configuration_id)
    ...
    def test_list_few_bucket_inventory(self):
        ...
        configuration_id_prefix = "test-id-"
        for index in range(0, 3):
            configuration_id = configuration_id_prefix + str(index)
            optional_fields = [FIELD_SIZE, FIELD_LAST_MODIFIED_DATE, FIELD_STORAG_CLASS,
                    FIELD_ETAG, FIELD_IS_MULTIPART_UPLOADED, FIELD_ENCRYPTION_STATUS]
