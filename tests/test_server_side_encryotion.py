...
class TestSSEDataEncryption(OssTestCase):
    ...
    def test_put_bucket_encryption(self):
        ...
        self.assertEqual(oss2.SERVER_SIDE_ENCRYPTION_SM4, result.sse_algorithm)
        ...
        result = bucket.get_bucket_encryption()
        self.assertEqual(oss2.SERVER_SIDE_ENCRYPTION_KMS, result.sse_algorithm)
    ...
    def inner_put_object_encryption_result(self, bucket, object_name, data, sse_algorithm, data_algorithm):
        headers = dict()
        if sse_algorithm:
    ...
    def test_put_object_with_encryption(self):
        ...
        data = b'a'
        sse_algo, data_algo, key_id = self.inner_put_object_encryption_result(bucket, object_name, data, None, None)
        self.assertIsNone(sse_algo)
        self.assertIsNone(data_algo)
        self.assertIsNone(key_id)

        object_name = 'test-put-object-kms'
        sse_algo, data_algo, key_id = self.inner_put_object_encryption_result(bucket, object_name, data, SERVER_SIDE_ENCRYPTION_KMS, None)
        self.assertEqual(SERVER_SIDE_ENCRYPTION_KMS, sse_algo)
        self.assertIsNone(data_algo)
        self.assertIsNotNone(key_id)

        object_name = 'test-put-object-aes'
        sse_algo, data_algo, key_id = self.inner_put_object_encryption_result(bucket, object_name, data, SERVER_SIDE_ENCRYPTION_AES256, None)
        self.assertEqual(SERVER_SIDE_ENCRYPTION_AES256, sse_algo)
        self.assertIsNone(data_algo)
        self.assertIsNone(key_id)

        object_name = 'test-put-object-sm4'
        sse_algo, data_algo, key_id = self.inner_put_object_encryption_result(bucket, object_name, data, SERVER_SIDE_ENCRYPTION_SM4, None)
        self.assertEqual(SERVER_SIDE_ENCRYPTION_SM4, sse_algo)
        self.assertIsNone(data_algo)
        self.assertIsNone(key_id)

        object_name = 'test-put-object-kms-sm4'
        sse_algo, data_algo, key_id = self.inner_put_object_encryption_result(bucket, object_name, data, SERVER_SIDE_ENCRYPTION_KMS, KMS_DATA_ENCRYPTION_SM4)
        self.assertEqual(SERVER_SIDE_ENCRYPTION_KMS, sse_algo)
        self.assertEqual(KMS_DATA_ENCRYPTION_SM4, data_algo)
        self.assertIsNotNone(key_id)

        object_name = 'test-put-object-aes256-sm4'
        sse_algo, data_algo, key_id = self.inner_put_object_encryption_result(bucket, object_name, data, SERVER_SIDE_ENCRYPTION_AES256, KMS_DATA_ENCRYPTION_SM4)
        self.assertEqual(SERVER_SIDE_ENCRYPTION_AES256, sse_algo)
        self.assertIsNone(data_algo)
        self.assertIsNone(key_id)

        object_name = 'test-put-object-sm4-sm4'
        sse_algo, data_algo, key_id = self.inner_put_object_encryption_result(bucket, object_name, data, SERVER_SIDE_ENCRYPTION_SM4, KMS_DATA_ENCRYPTION_SM4)
        self.assertEqual(SERVER_SIDE_ENCRYPTION_SM4, sse_algo)
        self.assertIsNone(data_algo)
        self.assertIsNone(key_id)

        object_name = 'test-put-object-none-sm4'
        sse_algo, data_algo, key_id = self.inner_put_object_encryption_result(bucket, object_name, data, None, KMS_DATA_ENCRYPTION_SM4)
        self.assertIsNone(sse_algo)
        self.assertIsNone(data_algo)
        self.assertIsNone(key_id)

    def inner_get_append_object_encryption_result(self, bucket, object_name, position, data, sse_algorithm, data_algorithm):
        headers = dict()
        if sse_algorithm:
            headers[OSS_SERVER_SIDE_ENCRYPTION] = sse_algorithm
        if data_algorithm:
    ...
    def test_append_object_with_encryption(self):
        ...
        sse_algo, data_algo, key_id = self.inner_get_append_object_encryption_result(bucket, object_name, 0, '123', SERVER_SIDE_ENCRYPTION_KMS, None)
        self.assertEqual(SERVER_SIDE_ENCRYPTION_KMS, sse_algo)
        self.assertIsNone(data_algo)
        self.assertIsNotNone(key_id)
        kms_key_id = key_id
        ...
        sse_algo, data_algo, key_id = self.inner_get_append_object_encryption_result(bucket, object_name, 3, '456', None, None)
        self.assertEqual(SERVER_SIDE_ENCRYPTION_KMS, sse_algo)
        self.assertIsNone(data_algo)
        self.assertIsNotNone(key_id)
        ...
        sse_algo, data_algo, key_id = self.inner_get_append_object_encryption_result(bucket, object_name, 6, '789', SERVER_SIDE_ENCRYPTION_AES256, None)
        self.assertEqual(SERVER_SIDE_ENCRYPTION_KMS, sse_algo)
        self.assertIsNone(data_algo)
        self.assertIsNotNone(key_id)
        self.assertEqual(kms_key_id, key_id)
