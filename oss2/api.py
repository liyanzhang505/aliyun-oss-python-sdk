...
class Bucket(_Base):
    ...
    def put_bucket_inventory_configuration(self, inventory_configuration):
        ...
        headers = http.CaseInsensitiveDict()
        headers['Content-MD5'] = utils.content_md5(data)
        resp = self.__do_bucket('PUT', data=data, params={Bucket.INVENTORY: '', Bucket.INVENTORY_CONFIG_ID:inventory_configuration.id}, headers=headers)

        return RequestResult(resp)

    def get_bucket_inventory_configuration(self, configuration_id=None):
        """获取指定的清单配置。

        :param str configuration_id : 清单配置id
        ...
        resp = self.__do_bucket('GET', params={Bucket.INVENTORY: '', Bucket.INVENTORY_CONFIG_ID:configuration_id})
    ...
    def delete_bucket_inventory_configuration(self, configuration_id):
        """删除指定的清单配置
        
        :param str configuration_id : 清单配置id
        :return: :class:`RequestResult <oss2.models.RequestResult>`
        """
        logger.debug("Start to delete bucket inventory configuration, bucket: {0}, configuration id: {1}.".format(self.bucket_name, configuration_id))
        resp = self.__do_bucket('DELETE', params={Bucket.INVENTORY:'', Bucket.INVENTORY_CONFIG_ID:configuration_id})
